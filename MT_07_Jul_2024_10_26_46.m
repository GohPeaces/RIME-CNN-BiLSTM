clc;clear;close all;	
load('MT_07_Jul_2024_10_26_46.mat')	
...
print_index_name=[]; 	
[xiangguan,~]=corr(A_data1(:,1:end-predict_num+1),'Type','Pearson'); %Pearson 相关性分析 	
index_name=data_biao1(1:end-predict_num+1);	
 y_index = index_name; x_index=index_name;  	
 figure(1)	
 heatmap(x_index,y_index,( xiangguan), 'FontSize',12, 'FontName','宋体'); 	
...
y_test_predict_norm=zeros(size(test_y_feature_label,1),size(test_y_feature_label,2));	
 ...
 [Mdl,~,fitness,Loss] = optimize_fitrCNN_BILSTM1_att(p_train1,train_y_feature_label_norm(:,list_cell{1,N1}),p_vaild1,vaild_y_feature_label_norm(:,list_cell{1,N1}),num_pop,num_iter,method_mti,max_epoch_LC,min_batchsize,attention_label,attention_head);  	
